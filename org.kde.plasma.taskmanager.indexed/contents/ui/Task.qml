...
PlasmaCore.ToolTipArea {
    ...
    PlasmaCore.DataSource {
        id: exec
        engine: "executable"          // ← позволяет запускать shell-команды
        onNewData: disconnectSource(sourceName)  // чтобы не засорять список
        // ↑ строка не обязательна, но убирает «висящие» job-и
        function run(cmdArray) {
            connectSource(cmdArray.join(" "));
        }
    }
